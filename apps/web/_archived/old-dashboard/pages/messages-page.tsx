          ...
          <div>
            <label className="block text-xs font-medium text-ink-60 mb-1.5 flex items-center gap-1">
              ...
              <span className="text-ink-40 font-normal">
                (needed for email notifications / SMS ΓÇö In-app alone skips email)
              </span>
            </label>
            <input
              value={clientId}
              onChange={(e) => setClientId(e.target.value)}
              placeholder="Paste client CUIDΓÇª"
          ...
          <div>
            ...
            {channels.length < 3 && (
              <p className="mt-1.5 text-xs text-ink-60">
                Some channels are off in Settings ΓåÆ Messaging.
              </p>
            )}
            {!clientId.trim() && (
        ...
        <div className="flex max-h-[min(640px,calc(100vh-160px))] min-h-[320px] flex-col overflow-hidden rounded-xl border border-ink-20 bg-white">
          {isLoading ? (
            <div className="flex items-center justify-center py-20 text-ink-60">
              ...
              <span className="text-sm">Loading messagesΓÇª</span>
            </div>
          ) : isError ? (
            <ApiErrorState
              error={error}
