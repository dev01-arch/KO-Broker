      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-ink-60 font-mono">{report.caseId.slice(0, 8)}ΓÇª</p>
          </div>
        </div>
        ...
        <div className="border-t border-ink-20 px-5 py-4 space-y-4">
          ...
          {(report.status === 'APPROVED' || report.status === 'FINALISED') && (
            <div className="flex items-center gap-2 rounded-lg bg-brand-teal-50 px-3 py-2 text-sm text-brand-teal-700">
              ...
              {report.approvedBy && <span className="text-brand-teal-600/70"> by {report.approvedBy.slice(0, 8)}ΓÇª</span>}
              {report.pdfUrl && (
                <a
                  href={report.pdfUrl}
                  target="_blank"
      ...
      <div className="p-7">
        {isLoading ? (
          <div className="flex items-center justify-center py-20 text-ink-60">
            ...
            <span className="text-sm">Loading reportsΓÇª</span>
          </div>
        ) : isError ? (
          <ApiErrorState
            error={error}
