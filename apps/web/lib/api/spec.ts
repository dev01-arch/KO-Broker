...
export const ENDPOINTS: EndpointDef[] = [
    ...
    {
        method: 'GET',
        path: '/api/cron/intelligence-rates',
        summary: 'Ingest BoE quoted rate series (GET / Vercel Cron)',
        description: 'Vercel Cron scheduled endpoint (daily at 07:00 UTC). Ingests the 5 official Bank of England benchmark rate series (IUMBV42 2yr fixed, IUMBV45 3yr fixed, IUMBV44 5yr fixed, IUMBV47 variable 75% LTV, and IUM2W42 effective new 2yr fixed). Upserts RateSeriesPoint and updates DataFeedStatus for BOE_RATES. Protected by CRON_SECRET in production.',
        auth: false,
        tags: ['Intelligence'],
        params: [
        ...
        responses: [
            { status: 200, description: 'Ingest succeeded', example: { ok: true, feedStatus: 'success', seriesIngested: 5, ranAt: '2026-09-08T07:00:00.000Z' } },
...

    {
        method: 'POST',
        path: '/api/cron/intelligence-rates',
        summary: 'Manually trigger BoE rate ingest',
        description: 'Manual or webhook trigger to immediately run the Bank of England rate ingest job. Idempotent. Protected by CRON_SECRET in production.',
        auth: false,
        tags: ['Intelligence'],
        params: [
            { name: 'authorization', in: 'body', required: false, type: 'string', description: 'Bearer <CRON_SECRET> header required in production', example: 'Bearer cron_secret_abc123' },
    ...
    {
        method: 'GET',
        path: '/api/cron/intelligence-prices',
        summary: 'Ingest HMLR house price aggregates (GET / Vercel Cron)',
        description: 'Vercel Cron scheduled endpoint (daily at 08:00 UTC). Queries HM Land Registry SPARQL endpoint for sold prices grouped by outward code (up to 50 outward codes per run). Computes median price, 12-month change %, and transaction count, and upserts into LocalPriceStat. Updates DataFeedStatus for HMLR_PRICES. Protected by CRON_SECRET in production.',
        auth: false,
        tags: ['Intelligence'],
        params: [
            { name: 'authorization', in: 'body', required: false, type: 'string', description: 'Bearer <CRON_SECRET> header required in production', example: 'Bearer cron_secret_abc123' },
        ...
        responses: [
            { status: 200, description: 'HMLR ingest completed', example: { ok: true, feedStatus: 'success', codesProcessed: 6, results: [{ outwardCode: 'SW1A', status: 'upserted', medianPrice: 1250000, txnCount: 48 }], ranAt: '2026-09-08T08:00:00.000Z' } },
            { status: 401, description: 'Unauthorized', example: { error: 'Unauthorized' } },
            { status: 503, description: 'CRON_SECRET required in production', example: { error: 'CRON_SECRET is required in production' } },
        ],
    },

    {
        method: 'POST',
        path: '/api/cron/intelligence-prices',
        summary: 'Manually trigger HMLR price ingest',
        description: 'Manual or webhook trigger to refresh HM Land Registry sold-price aggregates. Protected by CRON_SECRET in production.',
        auth: false,
        tags: ['Intelligence'],
        params: [
            { name: 'authorization', in: 'body', required: false, type: 'string', description: 'Bearer <CRON_SECRET> header required in production', example: 'Bearer cron_secret_abc123' },
        ...
        responses: [
            { status: 200, description: 'HMLR ingest completed', example: { ok: true, feedStatus: 'noop', codesProcessed: 0, results: [], ranAt: '2026-09-08T08:00:00.000Z' } },
            { status: 401, description: 'Unauthorized', example: { error: 'Unauthorized' } },
            { status: 503, description: 'CRON_SECRET required in production', example: { error: 'CRON_SECRET is required in production' } },
        ],
    },

    {
        ...
        responses: [
            {
                status: 200,
                description: 'Overview data with 4 rate cards and feed health',
                example: {
                    success: true,
                    data: {
                        rates: [
                            { seriesId: 'IUMBV42', label: '2yr Fixed (75% LTV)', value: 4.43, change12mBps: -48, asAt: '2026-09-01T00:00:00.000Z' },
                            { seriesId: 'IUMBV44', label: '5yr Fixed (75% LTV)', value: 4.18, change12mBps: -35, asAt: '2026-09-01T00:00:00.000Z' },
                            { seriesId: 'IUMBV47', label: 'Variable (75% LTV)', value: 5.75, change12mBps: -25, asAt: '2026-09-01T00:00:00.000Z' },
                            { seriesId: 'IUM2W42', label: 'Effective New (2yr Fixed)', value: 4.29, change12mBps: -41, asAt: '2026-09-01T00:00:00.000Z' },
                        ],
            ...
            { status: 401, description: 'Unauthorized', example: { success: false, error: { code: 'UNAUTHORIZED', message: 'You must be signed in to access this resource' } } },
            { status: 403, description: 'Forbidden — plan gate', example: { success: false, error: { code: 'FORBIDDEN', message: "Feature 'mortgage_intelligence' is not available on your plan." } } },
        ],
    },

    {
        ...
        summary: 'Current benchmark rates (Calculator context)',
        description: 'Slim payload for calculator panels and quick context widgets. Returns the three standard benchmark rates (fixed2yr, fixed5yr, variable75) and their asAt effective dates.',
        auth: true,
        tags: ['Intelligence'],
        responses: [
            {
                ...
                example: {
                    success: true,
                    data: {
                        fixed2yr: { value: 4.43, asAt: '2026-09-01T00:00:00.000Z' },
                        fixed5yr: { value: 4.18, asAt: '2026-09-01T00:00:00.000Z' },
                        variable75: { value: 5.75, asAt: '2026-09-01T00:00:00.000Z' },
                    },
                },
            },
        ],
...

    {
        ...
        summary: 'Case pre-flight preview (Confirm box)',
        description: 'Assembles Case + Client + FactFind figures for the pre-flight confirmation dialog. Every field returns { value, present } — missing data returns present: false, never £0 or fabricated numbers. Restricted advisers can only preview cases assigned to them.',
        auth: true,
        tags: ['Intelligence'],
        params: [
            { name: 'caseId', in: 'path', required: true, type: 'string', description: 'Target case ID', example: 'cmttb96si0006ns9vtlkurk6g' },
        ],
        responses: [
            {
                status: 200,
                description: 'Pre-flight case figures',
                example: {
                    success: true,
                    data: {
                        caseLabel: 'KO-INTEL-123456 — Jane Doe',
                        postcode: { value: 'SW1A 2AA', present: true },
                        propertyValue: { value: 450000, present: true },
                        deposit: { value: 130000, present: true },
                        mortgageAmount: { value: 320000, present: true },
                        termYears: { value: 25, present: true },
                        grossIncome: { value: 95000, present: true },
                        secondIncome: { value: null, present: false },
                        monthlyCommitments: { value: 600, present: true },
                    },
                },
            },
            { status: 404, description: 'Case not found or invisible to caller', example: { success: false, error: { code: 'NOT_FOUND', message: 'Case not found' } } },
        ],
    },

    {
        ...
        summary: 'Generate intelligence snapshot (Create)',
        description: 'Validates inputs using CreateSnapshotSchema, executes the formula engine, resolves geography via PostcodeGeography, generates non-advisory insight and watch paragraphs, and INSERT-ONLY records a CaseIntelligenceSnapshot. Subsequent generations create new rows to preserve an audit trail. Returns fully rendered SnapshotResponse.',
        auth: true,
        tags: ['Intelligence'],
        params: [
            { name: 'postcode', in: 'body', required: true, type: 'string', description: 'Full or outward UK postcode', example: 'SW1A 2AA' },
            { name: 'propertyValue', in: 'body', required: true, type: 'number', description: 'Property valuation (£)', example: 450000 },
            { name: 'mortgageAmount', in: 'body', required: true, type: 'number', description: 'Mortgage loan amount (£)', example: 320000 },
            { name: 'termYears', in: 'body', required: true, type: 'number', description: 'Mortgage term (1–40 years)', example: 25 },
            { name: 'source', in: 'body', required: true, type: 'string', description: 'Origin of figures', enum: ['MANUAL', 'CONFIRMED_FROM_CASE'] },
            { name: 'caseId', in: 'body', required: false, type: 'string', description: 'Required when source=CONFIRMED_FROM_CASE', example: 'cmttb96si0006ns9vtlkurk6g' },
            { name: 'confirmedAt', in: 'body', required: false, type: 'string', description: 'ISO timestamp (required when source=CONFIRMED_FROM_CASE)', example: '2026-09-08T23:45:00.000Z' },
            { name: 'deposit', in: 'body', required: false, type: 'number', description: 'Deposit amount (£)', example: 130000 },
            { name: 'grossIncome', in: 'body', required: false, type: 'number', description: 'Primary gross annual income (£)', example: 95000 },
            { name: 'secondIncome', in: 'body', required: false, type: 'number', description: 'Second applicant gross annual income (£)', example: 35000 },
            { name: 'monthlyCommitments', in: 'body', required: false, type: 'number', description: 'Monthly credit obligations (£)', example: 600 },
        ],
        responses: [
            {
                status: 201,
                description: 'Snapshot generated and stored',
                example: {
                    success: true,
                    data: {
                        id: 'cmttbk4vr000bnseaj8l3ey2x',
                        orgId: 'org_123',
                        caseId: 'cmttb96si0006ns9vtlkurk6g',
                        source: 'CONFIRMED_FROM_CASE',
                        generatedAt: '2026-09-08T23:47:12.183Z',
                        postcode: 'SW1A 2AA',
                        outwardCode: 'SW1A',
                        ...
                        dti: 7.58,
                        dtiBand: 'LOW',
                        monthlyPayment: 1765.97,
                        marketSignal: 'IMPROVING',
                        insightText: 'The loan-to-value ratio is 71.1%. At the 4.43% 2-year fixed benchmark (BoE, September 2026), an indicative repayment figure would be £1,766 per month over 25 years.',
                        watchText: null,
                        geography: {
                            region: 'London',
                        ...
                        outputsJson: {
                            benchmarkRate2yr: 4.43,
                            monthlyAt2yr: 1765.97,
                            benchmarkRate5yr: 4.18,
                            monthlyAt5yr: 1721.45,
                            medianPriceLocal: 1250000,
                            change12mPctLocal: 3.2,
                        },
                        sourcesJson: {
                            rates: { asAt: '2026-09-01T00:00:00.000Z', series: ['IUMBV42', 'IUMBV44', 'IUMBV47'] },
                            localPrices: { outwardCode: 'SW1A', asOf: '2026-09-08T00:00:00.000Z' },
                        },
                    },
                },
            },
            { status: 422, description: 'Zod Validation Error on payload', example: { success: false, error: { code: 'VALIDATION_ERROR', message: 'Request validation failed', fields: { caseId: ['caseId is required when source is CONFIRMED_FROM_CASE'] } } } },
            { status: 503, description: 'Benchmark rates not yet ingested into database', example: { success: false, error: { code: 'RATE_DATA_NOT_READY', message: 'Rate data is not yet available. Please trigger the BoE rates ingest cron.' } } },
        ],
    },

    {
        method: 'GET',
        path: '/api/intelligence/snapshots',
        summary: 'List snapshots history',
        description: 'Returns snapshots for the authenticated organization, optionally filtered by ?caseId=. Sorted newest first (ORDER BY generatedAt DESC).',
        auth: true,
        tags: ['Intelligence'],
        params: [
            { name: 'caseId', in: 'query', required: false, type: 'string', description: 'Filter snapshots by Case ID', example: 'cmttb96si0006ns9vtlkurk6g' },
        ],
        responses: [
            {
                status: 200,
                    ...
                    data: [
                        {
                            ...
                            monthlyPayment: 1765.97,
                            marketSignal: 'IMPROVING',
                            insightText: 'The loan-to-value ratio is 71.1%...',
                        },
                    ],
                },
            },
...

    {
        ...
        summary: 'Get snapshot by ID (Replay normalized response)',
        description: 'Fetches a single snapshot by ID for UI replay and audit inspection. Returns the normalized SnapshotResponse contract (ISO string dates, structured geography, typed outputsJson, and sourcesJson). Org-scoped — returns 404 if belonging to another organisation.',
        auth: true,
        tags: ['Intelligence'],
        params: [
            { name: 'id', in: 'path', required: true, type: 'string', description: 'Snapshot CUID', example: 'cmttbk4vr000bnseaj8l3ey2x' },
        ],
        responses: [
            {
                status: 200,
                description: 'Normalized snapshot contract',
                example: {
                    success: true,
                    data: {
                        id: 'cmttbk4vr000bnseaj8l3ey2x',
                        ...
                        sourcesJson: {
                            rates: { asAt: '2026-09-01T00:00:00.000Z' },
                            localPrices: { outwardCode: 'SW1A', asOf: '2026-09-08T00:00:00.000Z' },
                        },
                    },
                },
            },
            { status: 404, description: 'Snapshot not found in caller organisation', example: { success: false, error: { code: 'NOT_FOUND', message: 'Snapshot not found' } } },
        ],
    },

    {
        ...
        summary: 'Copy snapshot block to Case adviser notes',
        description: 'Appends a formatted J4 compliance block ([Mortgage Intelligence · Date], insight text, sources, and regulatory non-advice disclaimer) to Case.adviserNotes. Only available for case-linked snapshots. Logs an immutable audit event Case / INTELLIGENCE_COPIED_TO_NOTES.',
        auth: true,
        tags: ['Intelligence'],
        params: [
            { name: 'id', in: 'path', required: true, type: 'string', description: 'Snapshot CUID', example: 'cmttbk4vr000bnseaj8l3ey2x' },
        ...
        responses: [
            {
                status: 200,
                description: 'Notes successfully appended',
                example: {
                    success: true,
                    data: {
                        caseId: 'cmttb96si0006ns9vtlkurk6g',
                        snapshotId: 'cmttbk4vr000bnseaj8l3ey2x',
                    },
                },
            },
            ...
            { status: 409, description: 'Conflict — snapshot is not linked to a case', example: { success: false, error: { code: 'CONFLICT', message: 'This snapshot is not linked to a case. Copy to notes is only available for case-linked snapshots.' } } },
        ],
    },
];
